t301.ready(function() {
    console.table(titm_settings.settings);
    ...
    function standardizedName(img,type){
        ...
        var img_name = img.src.substring(0, (img.src.indexOf("#") == -1) ? img.src.length : img.src.indexOf("#"));
            ...
            if(parseInt(titm_settings.settings['strip_numbers'])==1){
            ...
            if(img_name.split("").length < 3){
                if(parseInt(titm_settings.settings['use_post_title_as_default'])==1){
                    if(titm_settings.settings['post_title']!==''){
                        img_name = titm_settings.settings['post_title'];
                    }
                    else{
                        img_name = titm_settings.settings['blog_name'];
                    }
                }
            }
            if(type=='title'){
