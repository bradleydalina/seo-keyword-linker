...
class __virgoSettingsCLS
{
    public function __construct()
    {
        if ( ! empty( $_POST ) && isset( $_POST['virgoSettingsForm'] ) )
        {
            $this->__virgoHandleForm();
        }
        $this->__virgoRenderForm();
    }

    public function __virgoHandleForm()
    {
    ...
    public function __virgoRenderForm()
    {
        ?>
        <form id="tag-settings" class="t-tabs t-show" method="POST">
            <h2>Plugin Settings</h2>
            <div class="t-wrap">
                    <?php wp_nonce_field( 'virgoSettingsUpdate', 'virgoSettingsForm' ); ?>
                    <input type="hidden" name="virgoSettingsUpdate" value=1 />
                    <br/>
...
$__virgoSettingsCLS = new __virgoSettingsCLS();
